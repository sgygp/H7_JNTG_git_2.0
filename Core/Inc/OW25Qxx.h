...
#define QUAD_IN_FAST_PROG_4_BYTE_ADDR_CMD    		0x32    /* 32bit地址的4线快速写入命令 */
#define QUAD_INOUT_FAST_READ_4_BYTE_ADDR_CMD 	0xEB    /* 32bit地址的4线快速读取命令 */
...
extern "C" u8 OW25QXX_disk_write(u8 *buf, u32 addr, u16 count);
extern "C" u8 OW25QXX_disk_read(u8 *buf, u32 addr, u16 count);
...
public:
	...
	static uint8_t WriteBuffer(uint8_t *_pBuf, uint32_t _uiWriteAddr, uint16_t _usWriteSize);
	static u8 ReadBuffer(uint8_t * _pBuf, uint32_t _uiReadAddr, uint32_t _uiSize);
	static void EraseSector(uint32_t address);
	static void MemoryMapped(void);
	static void EraseChip(void);
	static void WriteEnable(void);
	static void AutoPollingMemReady(u32 timeout_ms);
