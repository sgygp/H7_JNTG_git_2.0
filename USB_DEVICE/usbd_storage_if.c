...
extern uint8_t disk_read (uint8_t pdrv, uint8_t* buff, uint32_t sector, uint16_t count);
extern uint8_t disk_write (uint8_t pdrv, const uint8_t* buff, uint32_t sector, uint16_t count);
...
#define STORAGE_BLK_NBR                  0x4000//0x10000
...
const int8_t STORAGE_Inquirydata_HS[] = {/* 36 */
		...
		0x00,
		0x00,
		'S', 'T', 'M', '_', 'Y', 'G', 'P', '1', /* Manufacturer : 8 bytes */
		...
		0x00,
		0x00,
		'S', 'T', 'M', '_', 'Y', 'G', 'P', '2', /* Manufacturer : 8 bytes */
		'P', 'r', 'o', 'd', 'u', 'c', 't', ' ', /* Product      : 16 Bytes */
		' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ',
		'0', '.', '0','1'
};
 ...
 * @retval USBD_OK if all operations are OK else USBD_FAIL
 */
//extern uint32_t SD_Sector_Count;
...
int8_t STORAGE_GetCapacity_HS(uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
	...
	if(lun==0)
	{
		*block_num  = OW25Q_Sector_Count;
		*block_size = STORAGE_BLK_SIZ;
	}else if(lun==1)
	{
		*block_num  =0; //SD_Sector_Count;
		*block_size = STORAGE_BLK_SIZ;
...
int8_t STORAGE_Read_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
	...
	//  UNUSED(blk_addr);
	//  UNUSED(blk_len);
	disk_read(lun,buf,blk_addr,blk_len);
	bUSB=1;
	return (USBD_OK);
	/* USER CODE END 13 */
}
...
int8_t STORAGE_Write_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
	...
	//  UNUSED(blk_addr);
	//  UNUSED(blk_len);
	disk_write(lun,buf,blk_addr,blk_len);
	bGaiTime=1;
